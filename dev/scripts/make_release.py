...
#: dev/, where this script lives and where its output is written.
HERE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
#: The repository root, one level above: the program itself, its icon, the licence
#: and the storemode flag all live there, because the program runs from there.
ROOT = os.path.dirname(HERE)
RELEASE = os.path.join(HERE, "release")
SINGLE = os.path.join(HERE, "release-onefile")
BUILD = os.path.join(HERE, "dist", "gpumon")

# The program's modules are at the repository root, so the name in the archive
# comes from the program itself rather than from a string here that goes stale at
# every version - and so does the licence and the icon it packages.
...
def set_store_mode(enabled: bool) -> None:
    ...
    """
    path = os.path.join(ROOT, "storemode.py")
...
def build_executable(store: bool = False) -> bool:
    ...
    command = [
        ...
        "--icon", os.path.join(ROOT, "gpumon.ico"),
        "--add-data", os.path.join(ROOT, "gpumon.ico") + ";.",
        "--add-data", os.path.join(ROOT, "gpumon.png") + ";.",
        "--hidden-import", "psutil", "--hidden-import", "wmi",
        "--exclude-module", "tkinter.test", "--exclude-module", "test",
        os.path.join(ROOT, "gpumon.py"),
    ]
