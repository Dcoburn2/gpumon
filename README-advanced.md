...
> **Where things are.** The program is at the top of the repository: `gpumon.py`,
> the modules it imports, the interface in `ui/`, the per-OS backends in
> `platforms/`, and the launchers. Everything that exists only to build, test or
> package it is in `dev/` - the test suite, the packaging scripts, the Store
> submission material, and the build output they produce. Every command below is
> written from the repository root and works as typed.
...
```powershell
python dev/scripts/make_release.py --all      # every artifact, from one source tree
```
...
```powershell
python dev/scripts/make_release.py --build     # -> release\gpumon.exe, ready to run
python dev/scripts/make_release.py --all       # + the zip and the single-file build
```
...
after editing the artwork: `python dev/scripts/make_icon.py`.
...
```powershell
python dev/scripts/make_store_package.py                 # build store/gpumon-<version>-x64.msix
python dev/scripts/make_store_package.py --layout-only   # just the package folder
python dev/scripts/make_store_package.py --sign          # + a self-signed cert, for local testing
python dev/scripts/store_capture.py                      # the 1366x768 screenshots the Store wants
```
...
python dev/scripts/make_release.py --all           # signs what it built, then checksums it
python dev/scripts/make_release.py --sign          # or require signing: fail if it cannot
```
